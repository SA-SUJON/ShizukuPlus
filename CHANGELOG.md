...
- **Completed Android 16 VerifyError fix by targeting the shell module's separate R8 pass** — the previous two fixes added the `-keep` rule to `manager/proguard-rules.pro`, but class `e42` (identified by a stable `r8-map-id` across all builds) was not in the manager's R8 pass. The `shell/` module compiles independently: its R8 output is `rish_shizuku.dex`, bundled as an asset and loaded at runtime via `BaseDexClassLoader`. The shell's own `proguard-rules.pro` had `-repackageclasses` (causing the short name `e42`) but no keep rule for `rikka.hidden.compat.adapter.**`. Added the same `-keep class rikka.hidden.compat.adapter.** { *; }` rule to `shell/proguard-rules.pro`, which is the module actually processed by the failing R8 pass. ([#537](https://github.com/thejaustin/ShizukuPlus/issues/537), [#544](https://github.com/thejaustin/ShizukuPlus/issues/544))

#### Manager App (UI)
- **Fixed appearance settings crash** (`IllegalStateException: Preference already has a SummaryProvider set`) — `fixDeprecatedListPreferenceSummaries` could try to install `SimpleSummaryProvider` on a `rikka.preference.SimpleMenuPreference` that already had one installed (from its `useSimpleSummaryProvider="true"` XML attribute), crashing the Appearance fragment on open. Added a `summaryProvider == null` guard to skip already-initialized preferences. Also removed `useSimpleSummaryProvider="true"` from the `custom_accent` preference (which was silently overriding the fragment's hand-crafted `"Violet has been applied"` summary text with the raw entry key). ([#529](https://github.com/thejaustin/ShizukuPlus/issues/529))
- **Fixed home screen settings/help icons hidden behind the status bar on Xiaomi HyperOS (OS 15)** — `WindowInsets.statusBars` reports 0 on some Xiaomi devices, collapsing the top bar height and placing the icons under the real status bar where they can't be tapped. Changed to `WindowInsets.safeDrawing` (`systemBars ∪ displayCutout`) which correctly returns the top inset on all OEMs. ([#529](https://github.com/thejaustin/ShizukuPlus/issues/529))
...
### 🎨 UI / Visual Polish
- **Tinted copy and regenerate icons in the Home Automation "View Intents" sheet with `colorPrimary`** — the icons defaulted to `colorOnSurfaceVariant` (grey), making the interactive copy and regenerate-token buttons look inactive. Both end icons (copy) and start icon (regenerate) now use the app's primary accent color. ([#542](https://github.com/thejaustin/ShizukuPlus/issues/542))
- **Replaced full-screen settings search bar with an inline search experience** — the previous `SearchBar(active=true)` composable expanded to fill the entire screen ("monolith" look on Samsung One UI 8.5). The search icon now transforms the `TopAppBar` into a back button + inline `TextField`; results appear in the content area below. Eliminates the full-screen overlay and works predictably across OEM launchers. ([#538](https://github.com/thejaustin/ShizukuPlus/issues/538))
...
- **Replaced deprecated `TopAppBarDefaults.largeTopAppBarColors()`** with the unified `topAppBarColors()` in `SettingsScreen.kt` — `largeTopAppBarColors` was deprecated in Material3 1.3; `topAppBarColors()` is the single replacement for all app bar variants.
